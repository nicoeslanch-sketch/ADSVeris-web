...
- For new website leads, create the lead in an editable source status in the service pipeline, link the contact, wait briefly, and then move it to the operational first-contact status. Kommo rejects direct API creation in the system `Leads Entrantes` status; the delayed move gives Kommo stage-entry email triggers a clear move to `Contactado` after the contact email is available.
